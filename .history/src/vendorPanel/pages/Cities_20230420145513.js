    ...
    const fetchData = useCallback(async () => {
      try {
        const { data } = await axios.get(
          "http://ec2-65-1-248-95.ap-south-1.compute.amazonaws.com:7777/api/v1/cities",
          {
            headers: {
      ...
      const postHandler = async (e) => {
        ...
        try {
          const { data } = await axios.post(
            "http://ec2-65-1-248-95.ap-south-1.compute.amazonaws.com:7777/api/v1/admin/cities",
            {
              country , state :code , city
            },
            {
              headers: {
                Authorization: `Bearer ${token}`,
        ...
        <section>
          <div className="pb-4 sticky top-0  w-full flex justify-between items-center bg-white">
            <span className="tracking-widest text-slate-900 font-semibold uppercase ">
              All Cities ( Total : {data?.data?.length} )
            </span>
