  ...
  const fetchData = async () => {
    try {
      const { data } = await axios.get(
        "http://ec2-65-1-248-95.ap-south-1.compute.amazonaws.com:7777/api/v1/admin/legal/legalNotices"
      );
      setData(data);
    } catch (e) {
      console.log(e);
      ...
      <section>
        ...
        <div style={{ overflowX: "auto" }}>
          <Table striped bordered hover>
            ...
            <tbody>
                <tr >
                  <td> {data?.legal?.content} </td>
                  <td> {data?.legal?.createdAt?.slice(0, 10)} </td>
                  <td>
                    <i className="fa-solid fa-trash" onClick={() => deleteHandler(._id)} />
                  </td>
                </tr>
            </tbody>
          </Table>
