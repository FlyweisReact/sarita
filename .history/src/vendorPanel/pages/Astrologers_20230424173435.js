...
const Astrologers = () => {
  ...
  function MyVerticallyCenteredModal(props) {
    ...
    const [ pickupMan , setPickUpMan ] = useState("")
    ...
    const AssignHandler = async () => {
      try{
        const { data } = await axios.post("http://ec2-65-1-248-95.ap-south-1.compute.amazonaws.com:7777/api/v1/admin/assign-parcel" , {
          pickupMan , shippingOrderId : id
        })
        console.log(data)
        ...
        <Modal.Body>
          {edit ? (
            ...
            <Form onSubmit={AssignHandler}>
              <Form.Select
                className="mb-3"
                onChange={(e) => setPickUpMan(e.target.value)}
              >
                <option>-- Select Pick --</option>
                {each?.map((i , index) => (
