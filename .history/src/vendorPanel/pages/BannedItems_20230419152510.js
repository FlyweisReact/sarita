...
const BannedItems = () => {
  ...
  function MyVerticallyCenteredModal(props) {
    ...
    const postHandeler = async  () => {
        try {
            const  { data } = await axios.post("http://ec2-65-1-248-95.ap-south-1.compute.amazonaws.com:7777/api/v1/admin/items" , {bannedItems , restrictedItems})
            console.log(data)
            toast.success("Added")
            fetchData()
            props.onHide()
        ...
        <Modal.Body>
          <Form onSubmit={postHandeler}>
            <Form.Group className="mb-3">
              ...
              <Form.Control type="text"  onChange={(e) => setBannedItem(e.target.value)}/>
            </Form.Group>
    ...
    <>
      ...
      <section>
        <div className="pb-4 sticky top-0  w-full flex justify-between items-center bg-white">
          ...
          <button className="md:py-2 px-3 md:px-4 py-1 rounded-sm bg-[#19376d] text-white tracking-wider"   onClick={() => {
              setModalShow(true);
            }}>
            Add Banned Items
          </button>
