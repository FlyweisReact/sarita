...
const Astrologers = () => {
  ...
  function ViewModal(props) {
    ...
    const fetchParcel = useCallback(async () => {
      try { 
        const { data } = await axios.get(`http://ec2-65-1-248-95.ap-south-1.compute.amazonaws.com:7777/api/v1/admin/shippings/${id}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        })
      ...
      >
        ...
        <Modal.Body>
          <Container>
            ...
            <p className="View">
              {" "}
              <strong>Shipping From : </strong> {each?.data?.shippingFrom?.city + " " + each?.data?.shippingFrom?.city + " " + each?.data?.shippingFrom?.city + " , " + each?.data?.shippingFrom?.city}{" "}
            </p>
            <p className="View">
              {" "}
              <strong>Customer : </strong> {each?.data?.userId}{" "}
