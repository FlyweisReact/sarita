...
const BannedItems = () => {
  ...
  const delteHandler   =  async (id) => {
    try {
      const { data } = await axios.delete(`http://ec2-65-1-248-95.ap-south-1.compute.amazonaws.com:7777/api/v1/admin/items/${id}` ,    {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
          ...
          <tbody>
            {data?.data?.map((i, index) => (
              <tr key={index}>
                ...
                <td>
                  <i className="fa-solid fa-trash"  onClick={() => delteHandler(i._id)}/>
                </td>
              </tr>
            ))}
          </tbody>
