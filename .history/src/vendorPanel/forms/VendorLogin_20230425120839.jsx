...
const VendorLogin = () => {
  ...
  const submitHandler = async (e) => {
    ...
    setLoading(true);
    try {
      const { data } = await axios.post("http://ec2-65-1-248-95.ap-south-1.compute.amazonaws.com:7777/api/v1/admin/signin" , {
        email , password
      } )
    ...
    <>
      ...
      >
        <form className="shadow-2xl w-96 mx-3 sm:mx-0 sm:w-4/5 md:w-4/6 lg:w-4/5 xl:w-1/2 flex flex-col items-center bg-white p-5 md:py-10 ">
          <p className="text-3xl" style={{  color : "#19376d" , fontWeight : 'bold', textTransform : 'uppercase' }}>  Admin Panel </p>
          <section className="py-2">
          {setError === true ? <Alert variant="danger" >Check Your Email or Password  </Alert> :""}
